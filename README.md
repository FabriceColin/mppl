...
Since M3u8 playlists only include filenames, mpconv needs to know where tracks are on disk so that their metadata can be retrieved. On the Mac where the playlist was generated, the music collection was found at "/Volumes/PowerBook SD/Music" while it is mounted locally at "/fmedia/PowerBook SD/Music" so mpconv is called as follows.

```shell
$ mpconv -f "/Volumes/PowerBook SD/Music" -t "/fmedia/PowerBook SD/Music" Bought\ in\ 2021.m3u8 Bought\ in\ 2021
```
