...
{
	MusicCrawler crawler(topLevelDirName);
...
int main(int argc, char **argv)
{
	...
	while (optionChar != -1)
	{
		switch (optionChar)
		{
			case 'd':
				if (optarg != NULL)
				{
					MusicCrawler::m_maxDepth = (unsigned int)atoi(optarg);
				}
				break;
			case 'f':
				if (optarg != NULL)
	...
	if (argc - optind != 1)
	{
		clog << "Wrong number of parameters, expected COLLECTION_TOP_LEVEL_DIR" << endl;
		return EXIT_FAILURE;
	}
